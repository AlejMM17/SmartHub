...
export default function ActivitiesDialogCloseButton({ setFormData, formData, clickFunction, title, description }) {
    ...
    const handleStartDateChange = (e) => {
        ...
        const defaultEndDate = minEndDate.toISOString().split('T')[0];
    ...
    const handleDateChange = (range) => {
        setDateRange(range);
        if (range?.from) {
            const formattedStartDate = range.from.toISOString().split('T')[0];
            setStartDate(formattedStartDate);
            handleChangeFormData({ target: { name: 'startDate', value: formattedStartDate } });
        }
        if (range?.to) {
            const formattedEndDate = range.to.toISOString().split('T')[0];
            setEndDate(formattedEndDate);
            handleChangeFormData({ target: { name: 'endDate', value: formattedEndDate } });
        }
    };
